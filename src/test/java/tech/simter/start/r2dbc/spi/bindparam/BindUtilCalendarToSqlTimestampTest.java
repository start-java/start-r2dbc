 ...
 * @author RJ
 */
class BindUtilCalendarToSqlTimestampTest extends AbstractSpiTest {
  @Override
  ...
  void notNullValue() {
    ...
    connection()
      .flatMapMany(c -> c.createStatement("select value from t where id = $1")
        .bind("$1", id)
        .execute())
      .flatMap(result -> result.map((row, metadata) -> row.get("value", Calendar.class)))
      .as(StepVerifier::create)
      .expectNext(value)
      .verifyComplete();
  ...
  void nullValue() {
    ...
    connection()
      .flatMapMany(c -> c.createStatement("select value from t where id = $1")
        .bind("$1", id)
        .execute())
      .flatMap(result -> result.map((row, metadata) -> Optional.ofNullable(row.get("value", Calendar.class))))
      .as(StepVerifier::create)
